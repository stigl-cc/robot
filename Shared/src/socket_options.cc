...
bool SocketOptions::setKeepAlive(int fd, KeepAliveOptions options) {
    ...
    if(setsockopt(fd, SOL_TCP, TCP_KEEPCNT, &options.probes, sizeof(options.probes)) == -1) {
        log_tag_no(LOG_WARN, "set SO_KEEPCNT");
        return false;
    }

    if(setsockopt(fd, SOL_TCP, TCP_KEEPIDLE, &options.idleTime, sizeof(options.idleTime)) == -1) {
        log_tag_no(LOG_WARN, "set SO_KEEPIDLE");
        return false;
    }

    if(setsockopt(fd, SOL_TCP, TCP_KEEPINTVL, &options.interval, sizeof(options.interval)) == -1) {
